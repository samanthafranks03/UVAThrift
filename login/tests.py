...
# Test creation assisted by Microsoft Copilot
class RequestTestCase(TestCase):
    def test_sign_in_render(self):
        response = self.client.get(reverse("sign_in"))
        assert response.status_code == 200
        assert "sign_in.html" in [t.name for t in response.templates]

    def test_auth_receiver_missing_token(self):
    ...
    def test_auth_receiver_invalid_token(self):
        response = self.client.post(reverse("auth_receiver"), {"credential": "fake"})
...
class AccountCreationTestCase(TestCase):
    ...
    def test_new_user(self):
        ...
        self.assertEqual(test_user.picture,
